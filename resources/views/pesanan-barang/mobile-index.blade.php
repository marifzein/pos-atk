...
@section('content')
<div x-data="posBarangMobile()">

    <!-- SCROLLABLE BODY AREA -->
    <div class="flex-1 space-y-3.5 pb-48">
        
        <!-- 1. INPUT PENCARIAN / SCAN BARANG + TOMBOL CEK HARGA -->
        <div class="bg-white rounded-2xl p-3.5 shadow-xs border border-slate-100">
            <div class="flex items-center gap-2">
                <div class="relative flex-1" @click.outside="barangItems = []">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none">
                        <i class="ri-search-2-line text-slate-400 text-xl"></i>
                    </span>
                    <input
                        id="barangInputMobile"
                        x-ref="barangInputMobile"
                        type="text"
                        x-model="search"
                        @input="searchBarang"
                        @keydown.arrow-down.prevent="if(selectedIndex < barangItems.length - 1) selectedIndex++"
                        @keydown.arrow-up.prevent="if(selectedIndex > 0) selectedIndex--"
                        @keydown.enter.prevent="if(barangItems.length) addToCart(barangItems[selectedIndex])"
                    ...
                    <div x-show="barangItems.length" class="absolute left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-xl mt-1.5 z-40 max-h-60 overflow-y-auto divide-y divide-slate-100" x-cloak>
                        <template x-for="(barang, index) in barangItems" :key="barang.id">
                            ...
                            >
                                <div>
                                    <div class="font-bold text-slate-800 text-sm" x-text="barang.name"></div>
                                    <div class="text-xs font-mono text-slate-400" x-text="'Kode/Barcode: ' + (barang.barcode || barang.id)"></div>
                                </div>
                                <div class="text-right">
                                    <div class="text-sm font-black text-indigo-600" x-text="'Rp ' + formatRupiah(barang.price)"></div>
                                </div>
                            </div>
                        </template>
                    </div>
                ...
                <button @click="openPriceModal()" type="button" title="Cek Harga Barang" class="w-12 h-12 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl transition flex items-center justify-center active:scale-95 flex-shrink-0">
        ...
        <div class="bg-white rounded-2xl p-3.5 shadow-xs border border-slate-100">
            ...
            <div class="space-y-2.5">
                    ...
                    <div class="bg-slate-50 border border-slate-200/80 rounded-xl p-3 flex flex-col gap-2 shadow-2xs">
                        ...
                        <div class="flex justify-between items-start gap-2">
                            <div class="min-w-0">
                                ...
                                <div class="text-xs font-bold text-slate-400 mt-0.5" x-text="'Harga: @Rp ' + formatRupiah(item.harga)"></div>
                            </div>
                ...
                <div x-show="cart.length === 0" class="text-center py-8 text-slate-400 border border-dashed border-slate-200 rounded-xl bg-slate-50/50 text-sm">
                    Belum ada item barang dipilih.
                </div>
            </div>
        </div>

        <!-- 4. CATATAN PESANAN BARANG -->
        <div class="bg-white rounded-2xl p-3.5 shadow-xs border border-slate-100">
            <label class="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1">Catatan Pesanan Barang</label>
            <textarea x-model="catatan" rows="2" placeholder="Catatan pengiriman atau detail produk..." class="w-full bg-slate-50 border-0 focus:ring-2 focus:ring-indigo-500 text-sm rounded-xl p-3 outline-none"></textarea>
        </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-2xl p-4 z-40 rounded-t-2xl">
        <div class="max-w-md mx-auto space-y-2">
            ...
            <div class="bg-slate-50 rounded-xl p-2.5 border border-slate-100 flex items-center justify-between">
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wide block">Total Biaya Barang</span>
                    <span class="text-xl font-black text-slate-900 tracking-tight block truncate" x-text="'Rp ' + formatRupiah(subtotal)"></span>
                </div>
                <div class="text-right">
                    <span class="text-xs font-bold text-indigo-600 block bg-indigo-50 px-2 py-1 rounded-lg">WO BARANG</span>
                </div>
            </div>
            ...
            <button type="button" @click="saveTransaction()" class="w-full bg-indigo-600 active:bg-indigo-700 text-white font-black text-sm py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all">
                ...
                <span>SIMPAN PESANAN BARANG</span>
            </button>
        </div>
    </div>

    <!-- MODAL CEK HARGA BARANG -->
            ...
            <div class="mt-3 max-h-56 overflow-y-auto divide-y divide-slate-100 rounded-xl border border-slate-100">
                <template x-for="item in priceResults" :key="item.id">
                    <div class="p-3 bg-slate-50/50 flex justify-between items-center">
                        <div>
                            <div class="font-bold text-slate-800 text-sm" x-text="item.name || item.nama_barang"></div>
                            <div class="text-xs text-slate-400" x-text="'Kode/Barcode: ' + (item.barcode || item.id)"></div>
                        </div>
                        <div class="text-sm font-black text-indigo-600" x-text="'Rp ' + formatRupiah(item.price || item.harga)"></div>
                    </div>
                </template>
            </div>
        </div>
...
function posBarangMobile() {
    return {
        ...
        searchBarang() {
            ...
            if (q.length < 1) { this.barangItems = []; return; }
        ...
        async saveTransaction() {
            if (this.cart.length === 0) {
                Swal.fire({ icon: 'warning', text: 'Pilih minimal satu barang!' });
                return;
            }
