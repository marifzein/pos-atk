...
<x-card>
    ...
    <div class="flex flex-wrap justify-between items-center mb-6 gap-3">
        <form method="GET" action="{{ route('pesanan-jasa.history') }}" class="flex flex-wrap gap-3 items-center w-full md:w-auto">
        ...
        <x-table-header>
            <tr>
                <x-table-head class="p-3 text-left hover:bg-slate-200 transition cursor-pointer">
                    <a href="{{ $getSortLink('no_pesanan') }}" class="block w-full font-bold">
                        No WO <span class="text-indigo-600 text-xs">{{ $renderArrow('no_pesanan') }}</span>
                    </a>
                </x-table-head>

                <x-table-head class="p-3 text-left hover:bg-slate-200 transition cursor-pointer">
                    <a href="{{ $getSortLink('created_at') }}" class="block w-full font-bold">
                        Tanggal <span class="text-indigo-600 text-xs">{{ $renderArrow('created_at') }}</span>
                    </a>
                </x-table-head>
                ...
                <x-table-head class="p-3 text-left hover:bg-slate-200 transition cursor-pointer">
                    <a href="{{ $getSortLink('customer_id') }}" class="block w-full font-bold">
                        Pelanggan <span class="text-indigo-600 text-xs">{{ $renderArrow('customer_id') }}</span>
                    </a>
                </x-table-head>

                <x-table-head class="p-3 text-center hover:bg-slate-200 transition cursor-pointer">
                    <a href="{{ $getSortLink('status') }}" class="block w-full font-bold">
                        Status <span class="text-indigo-600 text-xs">{{ $renderArrow('status') }}</span>
                    </a>
                </x-table-head>
        ...
        <form @submit.prevent="submitBatal()">
            <div class="space-y-4">
                <div>
                    ...
                    <textarea x-model="alasan" rows="3" required placeholder="Masukan alasan pembatalan..." class="w-full border border-slate-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-red-200 focus:border-red-500 outline-none"></textarea>
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-600 mb-1">Password Otorisasi :</label>
                    <input type="password" x-model="password" required placeholder="Masukan password anda" class="w-full border border-slate-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-red-200 focus:border-red-500 outline-none">
                </div>
            </div>

            <div class="flex justify-end gap-3 mt-6">
                <button type="button" @click="showModal = false" class="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50">
                    Batal
                </button>
                <button type="submit" :disabled="loading" class="px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold text-sm transition flex items-center gap-2">
