...
@section('page_subtitle', 'Daftar transaksi Work Order')

@section('content')
<div x-data="historyJasaMobile()" class="space-y-3 pb-24">

    <!-- 1. TOOLBAR CARI & FILTER -->
    <div class="bg-white rounded-2xl p-3 shadow-xs border border-slate-100">
        <form method="GET" action="{{ route('pesanan-jasa.history') }}" class="space-y-2">
            ...
            @if(request()->hasAny(['search', 'customer_id', 'sort_by']))
                <div class="pt-1 flex justify-end">
                    <a href="{{ route('pesanan-jasa.history') }}" class="text-[11px] font-bold text-rose-500 hover:underline">
    ...
    <a href="{{ route('pesanan-jasa.index') }}" class="w-full bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 text-indigo-700 font-bold text-xs py-2.5 px-3 rounded-xl flex items-center justify-center gap-1.5 transition active:scale-98">
        <i class="ri-add-circle-line text-base"></i> Buat Pesanan Jasa Baru
    </a>
    ...
    <div class="space-y-2.5">
        @forelse($orders as $order)
            ...
            <div class="bg-white rounded-2xl p-3.5 shadow-xs border border-slate-100 space-y-2.5">
                ...
                <div class="space-y-1 text-xs">
                    ...
                    <div class="flex justify-between">
                        <span class="text-slate-400 font-medium">Layanan Jasa:</span>
        ...
        @empty
            <div class="text-center py-12 text-slate-400 italic text-xs bg-white rounded-2xl border border-dashed border-slate-200">
                ...
                Belum ada riwayat pesanan jasa.
            </div>
        @endforelse
    </div>
    ...
    <!-- MODAL PEMBATALAN PESANAN JASA (MOBILE FRIENDLY) -->
